...
\begin{example}[The Euclidean group]\index{Group!Euclidean}
    $\Or_n(\bbR)$ acts by automorphisms on the translation group $\bbR^n$. Their semidirect product is the Euclidean group $\Euc_n=\bbR^n\rtimes \Or_n(\bbR)$:
    \[(b,A)(b',A')=(n+An',AA').\]
...
\begin{example}[Lie group representations]
    \begin{enumerate}[label=(\alph*)]
        ...
        \item A faithful representation of the Euclidean group $\Euc_n$ is given by $\rho:\Euc_n\to \GL_{n+1}(\bbR)$ defined in block form by
...
\begin{proof}[Proof of Ehresmann's Fibration Theorem \ref{thm Ehresmann}]
    ...
    We will map the fibers $\pi^{-1}(\bbR^n)$ to the single fiber $\pi^{-1}(0)$ in a diffeomorphic way. This is done by moving the fibers along integral curves of some set of complete vector fields, in such a way that the flow takes each fiber to $\pi^{-1}(0)$. These vector fields are chosen to be $\pi$-related to some local sections of $TM$, so as to satisfy all the requirements. 
...
\begin{defn}[Gluing of manifolds]
    The construction described above is called the gluing of two manifolds with diffeomorphic boundaries, denoted by $M\cup_h N$.
\end{defn}

\begin{defn}[Connected sum]\index{Connected sum of manifolds}
    Let $M_i$, $i=1,2$ be two connected manifolds without boundaries and $M_i'=M_i\setminus U_i$, where $U_i$ is a coordinate ball. We can pick two diffeomorphisms $h_i:\partial M_i'\to \bbS^{m-1}$, $i=1,2$. Then we define the connected sum as the gluing along $h_1\circ h_2^{-1}$:
...
If $X\in\fX(M)$ and $X(p)\neq 0$, then there exists a local chart $(U,\varphi)$ at $p$ such that its slice $S=\{p\in U\mid \varphi^1(p)=0\}$ is diffeomorphic to $\bbD^{m-1}$, $X(p)\notin T_p S$ and $X=\partial_1$ on $U$.
\end{thm}
\begin{proof}
...
\begin{proof}
    ...
    Pick a local chart at $p$, $\varphi:U\to\wh{U}\subset\bbR^m$. Let $\bm{x}\in\wh{U}$, $\bm{a}=\varphi(p)$, $f\in C^\infty(M)$, then 
    \[(\varphi_\ast f)(\bm{x})=(\varphi_\ast f)(\bm{a})+\sum_{j=1}^m (x^j-a^j)\int_0^1 \frac{\partial(\varphi_\ast f)}{\partial x^j} (\bm{a}+t(\bm{x}-\bm{a}))\dd t\]
    on some neighborhood of $\bm{a}$.
    Therefore, for $u\in U$,
    \[f(u)=f(p)+\sum_{i=1}^m (\varphi^i(u)-a^i)g_i(u),\quad g_i(p)=\frac{\partial(\varphi_\ast f)}{\partial x^i} (\varphi(p)).\] 
    Thus,
...
The following shows that this definition of holonomic frames agrees with the one indicated in Example~\ref{def jet bundles}. That is, each holonomic frame arises from some local coordinates.
...
\begin{defn}[Exterior forms]\index{Exterior form}
    ...
    Both $\bigwedge^k$ and $\rmS^k$ are covariant functors $\mathsf{FinVect}_\bbK\to\mathsf{FinVect}_\bbK$ that act on morphisms by restrictions $\bigwedge^k f=\restr{f^{\otimes k}}{\bigwedge^k V}$ and $\rmS^k f=\restr{f^{\otimes k}}{\rmS^k V}$.
\end{defn}
