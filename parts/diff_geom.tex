...
\begin{defn}[Tensor bundles]\index{Tensor!bundles}
    ...
    \[
    T^r_s M\coloneqq \bbT^r_s (\T M)\cong (\T M)^{\otimes r}\otimes (\T^\ast M)^{\otimes s},\;\; T^\bullet_\bullet M\coloneqq \bigoplus_{r,s=0}^\infty T^r_s M.
    \]
...
\begin{thm}[{{\cite[Thm.~8.7]{Sharpe}}}]\label{thm 8.7 Sharpe}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item $\omega:\T M\to \frakg$ is an isomorphism on each fiber,
