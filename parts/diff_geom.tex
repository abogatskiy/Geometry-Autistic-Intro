...
\begin{rem}[Properties of $\theta_G$]
    \begin{enumerate}
        ...
        \[\dd\theta_G(X,Y)=-[X,Y]=-[\theta_G(X),\theta_G(Y)],\quad X,Y\in\fX(G),\]
        which can also be written as
        \[\dd\theta_G +\frac12 [\theta_G,\theta_G]=0.\label{eq 5.5.10 RS1 maurer-cartan}\]
        This is the Maurer-Cartan equation (\ref{eq 5.5.4 RS1 MC in coords}) in a basis-free form. It is also equivalent to the left-invariant analog of our original Maurer-Cartan equation (\ref{eq Maurer-Cartan left xi eta}), which is nothing but the pullback of this one along a map $g:[0,1]^2\to G$ (the right-invariant equation only differs in the relative sign of the two terms, see Remark~\ref{rem right MC eq}).
...
An element $a$ of a ring is called \emph{nilpotent}\index{Nilpotent element} if $a^n=0$ for some $n$ and \emph{unipotent} if $(a-1)$ is nilpotent.\index{Unipotent element}
