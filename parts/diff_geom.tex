...
\begin{rem}
    ...
    where $A_{\bf{e}_3}$ generates rotations about $\bf{e}_3$.
\end{rem}
\begin{cor}
...
is an analytic mapping $\frakg_e\to \Lin(\frakg)$ (the coefficients of its Taylor series are the Bernoulli numbers $B_k$). Let $\frakg_e^2$ be the set of $(A,B)\in \frakg\times\frakg_e$ such that the equation (first introduced by Poincar\'e in 1900)
...
\begin{multline}
    ...
    \right)(A),
\end{multline}
where the sum is taken over all $l_1,\ldots,l_k,m_1,\ldots,m_k\geq 0$ such that $l_j+m_j>0$, for all $j$. Integrating this over $t$ from $0$ to $1$ and using $Z(0)=B$, we find \emph{Dynkin's formula}\index{Dynkin's formula} (1947):
\begin{multline}
