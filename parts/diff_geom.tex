...
\begin{defn}[Integrable distributions]\index{Integrable distribution}
    A smooth distributions $D< TM$ is called integrable if every $m\in M$ is contained in a (local) integral manifold of $D$.
\end{defn}
...
Note also that $U$ gets decomposed into a disjoint union of integral submanifolds $U_{\bm{c}}$ parametrized by a point $\bm{c}\in S$ with coordinates $(0,\ldots,0,c^1,\ldots,c^{n-r})$ in the flat chart and defined as the slices $s^{r+1}=c^1,\ldots, s^n=c^{n-r}$.
...
\begin{proof}
    ...
    Let $V$ be a component of $N\cap U$. Since the chart is flat, the functions $x^{r+1},\ldots,x^n$ are constant along $D$ and therefore constant on any connected slice like $V$, and $V$ lies in a single slice $U_{\bm{c}}$, see Figure~\ref{fig: frobenius2}.

    Since $U_{\bm{c}}$ is embedded in $M$, the inclusion $V\hookrightarrow M$ ialso restricts to a smooth map into $U_{\bm{c}}$. The inclusion $V\hookrightarrow U_{\bm{c}}$ is thus an injective smooth immersion between manifolds of the same dimension, and therefore a local diffeomorphism, an open map, and a homeomorphism onto an open subset of $U_{\bm{c}}$. The inclusion map $V\hookrightarrow M$ is a composition of the smooth embeddings $V\hookrightarrow S\hookrightarrow M$ and thus a smooth embedding.
\end{proof}
...
\begin{proof}
    ...
    \[y^i=F^i(x),\]
    the fact that $F(B)\subset N\cap U$ means that the functions $F^{r+1},\ldots, F^n$ take on only countably many values. Since $B$ is connected, these coordinate functions are constant, and thus $F(B)$ lies in a single slice $U_{\bm{c}}\subset U$. Because $U_{\bm{c}}\cap N$ is an open subset in $N$ that is embedded in $M$, it follows that $\restr{F}{B}$ is smooth from $B$ into $U_{\bm{c}}\cap N$, and thus by composition, $\restr{F}{B}:B\to U_{\bm{c}}\cap N\hookrightarrow N$ is smooth into $N$.
\end{proof}
\begin{cor}
...
\begin{lem}[{{\cite[Lem.~3.5.14]{RS1}}}]\label{lem 3.5.14 RS1}
    ...
    \begin{enumerate}
        ...
        \item Let $N\subset M$ be an integral manifold of $D$, let $m\in M$ such that $\dim D_m=\dim N$ and let $(U,\kappa)$ be a local flat chart for $D$ at $m$. If a slice $U_{\bm{c}}$ of $U$ intersects $N$, then $U_{\bm{c}}$ is an integral manifold of $D$ and $N\cap U_{\bm{c}}$ is open and closed w.r.t.~the topology induced from $N$. In particular, the number of slices of $U$ intersecting $N$ is at most countable.  
    \end{enumerate}
\end{lem}
...
\begin{enumerate}
    ...
    \[\Phi^{(i)}:(-\epsilon,\epsilon)^r\to U\cap N_i,\quad \Phi^{(i)}(\bm{t})\coloneqq F^{X^{(i)}_1}_{t_1}\circ \cdots\circ F^{X^{(i)}_r}_{t_r}(m),\quad i=1,2.\]
    By construction these coordinates will coincide, $\Phi^{(1)}(\bm{t})=\Phi^{(2)}(\bm{t})$ (cf.\ Remark~\ref{rem 3.2.9 RS1}), and therefore their values will cover the same subset $V\subset N_1\cap N_2$ open in both $N_i$'s. W.r.t.~the differentiable structure induced from either $N_i$, $V$ is diffeomorphic to $(-\epsilon,\epsilon)^r$. Since the construction works for any $m\in N_1\cap N_2$, the assertion follows. 

    \item Let $r=\dim N=\dim D_m$. If $N$ intersects a slice $U_{\bm{c}}$, then $U_{\bm{c}}$ contains a point where $\rank D=r$. Then $U_{\bm{c}}$ is an integral manifold of $D$ by dimension-counting. Now assertion 1 implies that $N\cap U_{\bm{c}}$ is open in $N$ and hence in $N\cap U$. Since this holds for all slices intersecting $N$ and since
    \[N\cap U_{\bm{c}}=(N\cap U)\setminus\left(\bigcup_{\bm{c}'\neq \bm{c}}N\cap U_{\bm{c}'}\right),\]
    the intersection $N\cap U_{\bm{c}}$ is also closed in $N\cap U$. It follows that $N\cap U_{\bm{c}}$ is a union of connected components of $N\cap U$. Since the slices are disjoint, this implies that the number of slices which intersect $N$ is at most as large as the number of connected components of $N\cap U$. Since the topology on $N\cap U$ is induced from the manifold $N$, it is second countable, hence the number of connected components is at most countable.
\end{enumerate}
\end{proof}
...
\begin{example}[Zero curvature conditions]
    Another important case is the following system on an unknown function $\bm{y}:\bbR^2\to \bbC^n$ parametrized as $\bm{y}(x,t)$:
    \[\begin{cases}
        \partial_x \bm{y}(x,t)= U(x,t)\bm{y}(x,t),\\
        \partial_t \bm{y}(x,t)= V(x,t)\bm{y}(x,t),
    \end{cases}\label{eq UV pair}\]
    where $U,V:\bbR\times\bbR\to \bbC^{n\times n}$ are given matrix-valued functions. $U$ and $V$ can be viewed as defining a pair of $(x,t)$-dependent linear vector fields on $\bbC^n$. The Frobenius integrability condition in this case is easy to obtain by observing that any solution must satisfy $\partial_t(\partial_x \bm{y})=\partial_x(\partial_t \bm{y})$, which expands to 
    \[(\partial_t U+UV)\bm{y}=(\partial_x V+VU)\bm{y},\]
    or (since the initial condition is arbitrary)
    \[\partial_t U-\partial_x V+[U,V]=0.\]
    If this condition holds, then the Frobenius theorem guarantees the unique existence of solutions. In the theory of dynamical systems this condition is sometimes called a \emph{zero curvature condition}, and when a given dynamical system can be expressed in the form of a zero curvature condition, the system~(\ref{eq UV pair}) is called a \emph{$UV$-pair}, and the dynamical system is considered \emph{integrable}.\footnote{However, this is not the only test for integrability, and a general definition of integrability for dynamical systems doesn't exist} We will see its relation to the Maurer-Cartan equation in \subsect \ref{subsec: invariant diff forms}.
\end{example}

\begin{example}[Sine-Gordon equation]\label{ex sine-gordon}
    Consider the $UV$-pair 
    \[U(x,t;z)=\frac{\rmi}{2}\begin{pmatrix}
        2z & u_x\\
        u_x & -2z
    \end{pmatrix},\quad 
    V(x,t;z)=\frac{1}{4\rmi z}\begin{pmatrix}
        \cos u & -\rmi \sin u\\
        \rmi \sin u & -\cos u
    \end{pmatrix},
    \]
    where $u(x,t;z)$ is an $\bbR$-valued function, subscripts stand for partial derivatives of $u$, and $z\neq 0$ is a complex parameter. This $UV$-pair defines a system of equations (\ref{eq UV pair}) on the unknown $\bm{y}$. By expanding the zero curvature condition we find that its diagonal components vanish, and both anti-diagonal components impose the \emph{Sine-Gordon equation}\index{Equation!Sine-Gordon} on $u$:
    \[u_{xt}=\sin u.\]
\end{example}
...
\begin{example}[Nonlinear Schr\"odinger equation]
    ...
    \[
    U(x,t;z)=\rmi\begin{pmatrix}
        z & \wb\psi\\
        \psi & -z
    \end{pmatrix},\quad 
    V(x,t;z)=2\rmi\begin{pmatrix}
        z^2-\frac12|\psi|^2 & z\wb{\psi}-\frac{\rmi}{2}\wb{\psi}_x\\
        z{\psi}+\frac{\rmi}{2}{\psi}_x & -z^2+\frac12|\psi|^2.
    \end{pmatrix},
    \]
    where $\psi(x,t)\in\bbC$ and $\wb\psi$ is its complex conjugate. The zero curvature condition then leads to the \emph{Nonlinear Schr\"odinger} (NLS) equation on $\psi$, of great importance in optics:\index{Equation!Nonlinear Sch\"odinger}
...
\begin{example}[Lax pairs]\label{ex Lax pairs}\index{Lax pair}
    ...
    \[\partial_t M(t,s)=A(t)M(t,s),\quad M(s,s)=I_n,\]
    ...
    \[\bm{y}=(\bm{\psi},\partial_x\bm{\psi},\ldots,\partial_x^{n-1}\bm{\psi}),\]
    the equation $L\bm{\psi}=\lambda\bm{\psi}$ can be written as 
    \[\partial_x\bm{y}=U\bm{y},\]
    where 
    \[U(x,t;\lambda)=
    \begin{pmatrix}
        0 & 1 & 0 & \cdots & 0\\
...
\begin{example}
    ...
    The following choice of $A$ leads to the celebrated solitonic \emph{Korteweg-de~Vries} (KdV) equation:\index{Equation!Korteweg-de~Vries}
    \[A=-4\partial_x^3+6u\partial_x+3u_x\quad \Rightarrow\quad u_t-6uu_x+u_{xxx}=0.\]
\end{example}
...
\[\wt\dd \omega =\sum_{i_0=1}^n\sum_{1\leq i_1<\cdots <i_p\leq n}\partial_{i_0}\omega_{i_1 i_2\cdots i_p}\cdot \dd x^{i_0}\wedge \dd x^{i_1}\wedge\cdots\wedge \dd x^{i_p}.\]
...
On the other hand, $\int_{\partial U}\omega=\int_{\partial \bbR^n_+}\omega=\int_{\partial \bbR^n_+}\omega(x^1,\ldots,x^{n-1},0)\dd x^1\cdots \dd x^{n-1}$ and the outward normal on $\partial U$ is $-e_n=(0,\ldots,0,-1)$, therefore the induced orientation is $\sign(-e_n,e_1,\ldots,e_{n-1})=(-1)^n$, so
\[\int_{\partial U}i^\ast\omega=(-1)^{n}\int_{\bbR^{n-1}}\omega_n(x^1,\ldots,x^{n-1},0)\dd x^1\cdots\dd x^{n-1}=\int_U \dd\omega.\]
\end{proof}
\begin{cor}
If $M$ is compact and $\partial M=\varnothing$, then for any $\alpha\in\Omega^p(M)$, $\beta\in\Omega^{n-p}(M)$, and $X\in\fX(M)$ the following version of integration by parts holds:
