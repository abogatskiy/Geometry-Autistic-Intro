...
\begin{defn}[Integral manifolds]\index{Integral manifold}
    Let $D< TM$ be a smooth distribution (i.e.\ a subbundle of $TM$). An immersed submanifold $N<M$ is called an integral manifold of $D$ if $T_p N=D_p$ for all $p\in N$.
\end{defn}

\begin{defn}[Integrable distributions]\index{Integrable distribution}
    A smooth distributions $D< TM$ is called integrable if every $p\in M$ is contained in a (local) integral manifold of $D$.
\end{defn}
...
\begin{prop}[{{\cite[Prop~19.16]{Lee}}}]\label{prop 19.16 Lee}
    Let $D$ be an involutive distribution of rank $k$ on a smooth manifold $M$, and let $(U,\{x^i\})$ be a flat chart for $D$. If $H$ is an integral manifold of $D$, then $H\cap U$ is a union of countably many disjoint open subsets of parallel $k$-dimensional slices of $U$, each of which is open in $H$ and embedded in $M$.
\end{prop}
\begin{proof}
...
Recall now that a weakly embedded submanifold $H$ is such that any smooth map into $M$ whose values lie in $H$ restricts to a smooth map into $H$.
...
\begin{proof}
\begin{enumerate}
    Note that the number of slices of $U$ intersected by $N$ may be countably infinite, as is the case with a line of ``irrational slope'' on the torus.
