...
Note that if $M$ is not simply connected, then $L$ is still a Lie group because \emph{any} covering space of a Lie group is a Lie group, namely a quotient of the universal covering group of $G$ by some central discrete (and hence normal) subgroup. In this picture, developments of $\omega$ correspond to unique liftings of curves in $M$ to curves in $L$ and then projection to $G$.
