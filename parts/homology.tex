...
\begin{thm}[Poincar\'e lemma]\label{Poincare lem}
    ...
    \[H^{\smbullet }_{\rm dR}(M\times\bbR)=H^{\bullet}_{\rm dR}(M).\]
...
\begin{proof}
    ...
    \[s_0^\ast=s_1^\ast\implies f^\ast=g^\ast \text{ in }H^{\smbullet }_{\rm dR}.\]
\end{proof}
...
Obviously, for compact manifolds $H^{\smbullet }_c=H^{\smbullet }_{\rm dR}$, so in this case we have two \gls{mv} sequences in both directions!
...
\begin{example}[Dolbeault cohomology of the Riemann sphere]
    ...
    \begin{enumerate}
        \item $H^{0,0}(\CP^1)=\bbC$ (the only holomorphic functions on the entire sphere are the constant functions, by the Liouville theorem);
        \item $H^{0,1}(\CP^1)$ consists of 1-forms $f\dd\bar z$ (all of which are $\bar\partial$-closed for any $f\in C^\infty(\CP^1,\bbC)$) that are not exact, i.e.\ $f\neq \partial_{\bar z} g$ for any $g\in C^\infty(\CP^1,\bbC)$. Every such $f$ can be identified with a function on $\bbC$ such that the limit of $f \bar z^2$ as $z\to\infty$ exists. By Poincar\'e lemma there exists a $g$ such that $f=\partial_{\bar z} g$ and clearly it follows that $\lim_{z\to\infty}(\bar zg)$ exists, which makes $g$ a well-defined function on the sphere. Therefore $H^{0,1}(\CP^1)=0$;
        \item $H^{1,0}(\CP^1)$ consists of \emph{holomorphic} 1-forms $f\dd z$, which means that $f$ has to be holomorphic such that $\lim_{z\to\infty}z^2f$ exists. However, the only holomorphic function with such a limit is zero. Therefore $H^{1,0}(\CP^1)=0$;
        \item $H^{1,1}(\CP^1)$ consists of 2-forms $f\dd z\wedge\dd\bar z$. Such a form is exact iff $\int_{\CP^1}f\dd z\wedge\dd \bar z=0$. This is exactly one linear constraint, which means that $H^{1,1}(\CP^1)=\bbC$.
    \end{enumerate}
    With more tools, one can show that $H^{p,q}(\CP^n)$ is in general zero for $p\neq q$ and $H^{p,p}(\CP^n)=\bbC$ for $p=0,\ldots,n$.
\end{example}
...
\begin{defn}[(Co)cycles, (co)boundaries, (co)homologies of complexes]\index{Homology!of a chain complex}\index{Cohomology!of a cochain complex}
    ...
    \[Z^n(C^{\smbullet },d)=\ker d_n,\quad\quad B^n(C^{\smbullet },d)=\im d^{n-1},\quad\quad H^n(C^{\smbullet },d)=Z^n(C^{\smbullet },d)/B^n(C^{\smbullet },d).\]
\end{defn}
...
\[H_k(\RP^{2n};R)=\begin{cases}
    R,&k=0,\\
    0,&k\neq 0,
\end{cases}\quad
H_k(\RP^{2n+1};R)=\begin{cases}
    R,&k=0,n,\\
    0,&k\neq 0,n.
\end{cases}
\]
...
Now let $A$ be a $G$-module. $H^{\smbullet }(X,A)$ is defined as the homology of the complex $\Hom_G(C_\bullet(X),A)$. If $X$ is acyclic and $G$ acts freely, then $H^{\smbullet }(G,A)$ is the homology of $\Hom_G(S_\bullet(X),A)$ by Theorem~\ref{thm 9.45 Rotman}. 
