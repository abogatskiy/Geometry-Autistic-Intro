...
We end the \sect\ by showing that for a simply connected Lie group, its automorphism group is isomorphic to that of its Lie algebra.
...
\begin{lem}[{\cite[Lem.~9.2.26]{HN}}]\label{lem 9.2.26 HN}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \begin{enumerate}[label=(\alph*)]
            \item $\Lie\circ\alpha:G\to \Aut(\frakn)$ is smooth as a map into $\GL(\frakn)$;
...
\end{lem}
\begin{proof}
    (i) If $\sigma$ is smooth, then (b) is clearly satisfied. Moreover, $\exp_N\circ\Lief(\alpha(g))=\alpha(g)\circ\exp_N$ holds for each $g\in G$, so that the map $G\times\frakn\to\frakn$, $(g,A)\mapsto \Lief(\alpha(g))A$ is smooth on an open neighborhood of $(e,0)$. This implies that $\Lie\circ\alpha:G\to \GL(\frakn)$ is smooth in an open neighborhood of $e$, hence smooth because it is a group homomorphism.
...
\begin{cor}[{\cite[Cor.~9.5.11]{HN}}]\label{cor 9.5.11 HN}
    ...
    \[\Lie:\; \Aut(G)\to \Aut(\frakg), \quad \varphi\mapsto \varphi_{\ast e}\]
    is an isomorphism of groups. As a closed subgroup of $\GL(\frakg)$, the group $\Aut(\frakg)$ carries a natural Lie group structure, and we endow $\Aut(G)$ with the Lie group structure for which $\Lie$ is an isomorphism of Lie groups. Then the action $\Aut(G)\acts G$ is smooth.
\end{cor}
\begin{proof}
    First, we recall from Proposition~\ref{prop 1.1.3 DK} that for each automorphism $\varphi\in\Aut(G)$ the endomorphism $\Lief(\varphi)=\varphi_{\ast e}$ of $\frakg$ is also automorphism. That $\Lie$ is injective follows from the connectedness of $G$ by the First Principle from Remark~\ref{rem Lie's principles} and that $\Lie$ is surjective from Lie's Second Theorem~\ref{thm second principle}.

    If we endow $\Aut(G)$ with the Lie group structure for which $\Lief$ is an isomorphism, then the smoothness of the action $\Aut(\frakg)\acts \frakg$ and Lemma imply that the action $\Aut(G)\acts G$ is smooth.
\end{proof}
...
\section{(*) Nilpotent and solvable Lie groups}
...
\begin{prop}[{\cite[Prop.~11.1.19]{HN}}]\label{prop 11.1.19 HN}
    Let $G$, $H$, and $N$ be simply connected Lie groups with Lie algebras $\frakh,\frakh$, and $\frakn$. Suppose that $\frakg\cong\frakh\ltimes_\beta\frakn$ is a semidirect sum for some morphism $\beta:\frakh\to \Der(\frakn)$ (recall that $\Der(\frakn)<\frakgl(\frakn)$ is the Lie algebra of the full automorphism group $\Aut(\frakn)<\GL(\frakn)$). Then there is a unique smooth action $\gamma:H\to \Aut(N)\cong \Aut(\frakn)$ with $\gamma_{\ast e}=\beta:\frakh\to \Der(\frakn)$ and the two natural homomorphisms $i_H:H\to G$, $i_N:N\to G$ combine to an isomorphism 
...
The following fundamental result establishes the relationship between the nilpotency or solvability of Lie groups and their Lie algebras. Unfortunately, its proof requires some structure theory that we have not yet developed.
...
% \begin{proof}
    ...
    % \end{lem}
    % \begin{proof}
    %     From our assumptions, it immediately follows that $\frakh+\frakk+\frakl$ is a subalgebra of $\frakg$, and we may assume that it coincides with $\frakg$. Then $\frakl$ is an ideal of $\frakl$. First, we assume that $G$ is simply connected. Now $L$ is a closed subgroup of $G$ by Theorem~\ref{thm 11.2.21 }
    % \end{proof}
% \end{proof}
...
\begin{proof}
    ...
    \[yx(w) = xy(w) - [x,y](w) \in x\frakh(w) + \frakh(w) = \{0\}\]
...
Ado's Theorem (which we don't yet have the necessary structure theory to prove) states that every finite-dimensional real or complex Lie algebra can be embedded into a matrix algebra $\frakgl(V)$. Thus, combining the above two results, we conclude that nilpotent finite-dimensional Lie algebras can be represented by strictly triangular matrices: $\frakg\cdot V_i\subset V_{i-1}$.
...
\begin{defn}[Nilradical]\index{Nilradical}
    The nilradical of a finite-dimensional Lie algebra is the maximal nilpotent ideal, denoted by $\fraknil(\frakg)$.
\end{defn}
