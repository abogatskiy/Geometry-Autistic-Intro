...
\begin{enumerate}
    ...
    This can be seen by evaluating both sides on the basis elements $\bf{t}_i$. This equation is actually another form of the Maurer-Cartan equation \index{Equation!Maurer-Cartan} \eqref{eq Maurer-Cartan left xi eta}, which we will return to in a basis-free form later in this \sect.
    \item In terms of left-invariant 1-forms, the inverse left and right trivializations $\chi_\rmL^{-\sfT}$ and $\chi_\rmR^{-\sfT}$ of $\T^\ast G$ read
    \[\chi_\rmL^{-\sfT}(g,\xi)=\xi_g,\quad \chi_\rmR^{-\sfT}(g,\xi)=(\Ad_{g}^\ast \xi)_g=\xi_g\circ (\Ad_g)_{\ast g}.\]
...
\begin{cor}
    On every Lie group there exists a left-invariant volume form $\sfv_G$.\footnote{Invariant volume forms on Lie groups are also sometimes denoted by $\dd g$, but we will avoid this notation due to its likely confusion with the identity tensor on $\T G$.} This form is unique up to multiplication by a nonzero scalar. 
\end{cor}
...
\begin{rem}\label{rem biinvariant measures}
    \begin{enumerate}
        ...
        \item Naturally, there is also a right-invariant volume form $\sfw_G$, unique up to multiplication by a scalar, and a measure $\dd\mu_\rmR$ associated with it. The natural question thus is how the two Haar measures are related. Up to a multiplicative constant, we have $\rmL_g^\ast (\sfv_G)_g=(\sfv_G)_e=(\sfw_G)_e=\rmR_g^\ast (\sfw_G)_g$, so
        \[(\sfw_G)_g=\Adg_{g}^\ast (\sfv_G)_g=\det(\Ad_g)(\sfv_G)_g,\]
        where the last equality follows from $\Adg_{g\ast g}(A_\rmL)_g=(A_\rmR)_g=((\Ad_g A)_\rmL)_g$, hence $\det\Adg_{g\ast g}=\det\Ad_g$.
...
\begin{rem}[Properties of $\theta_G$]
    \begin{enumerate}
        ...
        This is the Maurer-Cartan equation \eqref{eq 5.5.4 RS1 MC in coords}\index{Equation!Maurer-Cartan} in a basis-free form. It is also equivalent to our original Maurer-Cartan equation \eqref{eq Maurer-Cartan left xi eta}, which is nothing but the pullback of this one along a map $g:[0,1]^2\to G$.
...
\begin{example}[Linear ODEs of Lie type]\index{Equation!of Lie type}
...
\begin{proof}
    ...
    % We will use the Maurer-Cartan equation \eqref{eq Maurer-Cartan}. Using the definition of $\xi$ and $\eta$ from there, we see
    % \[\eta(0,s)=\eta(1,s)=0.\]
    ...
    % \[\varPhi(g_1g_2)=h(1)=h_1(1)h_2(1)=\varPhi(g_1)\varPhi(g_2).\]
    % By a change of the parameter, we obtain that any path $\gamma(t)\in G$ with velocity $\xi(t)$ and initial condition $\gamma(0)=e$ is take by the map $f$ to a path $h(t)\in H$ with velocity $\phi\cdot \xi(t)$ and initial condition $h(0)=e$. Consequently, $\varPhi$ is differentiable and $f_{\ast e}=\phi$.
...
\end{proof}
\begin{rem}\label{rem Lie's principles}
...
This gives us a new interpretation of the Lie bracket as the  commutator of infinitesimal flows of left-invariant vector fields (closely connected to formula \eqref{30071}).
\begin{cor}
...
%     \[\varLambda(G)=\{\gamma\in \rmP(G,e)\mid \gamma(1)=e\}\]
%     is a normal subgroup of $\rmP(G,e)$ called the \emph{loop group} of $G$, while
%     \[\varLambda(G)^\circ=\{\gamma\in \rmP(G,e)\mid \gamma\sim \underbar{e}\},\]
%     which is the path component of the constant loop $\underbar{e}$, is closed and normal in $\rmP(G,e)$ as well. Now $\gamma\sim\dot\gamma$ in $\rmP(G,e)$ iff $\dot\gamma\in \gamma\varLambda(G)^\circ$, so that the universal covering space
%     \[\wt{G}=\rmP(G,e)\slash\varLambda(G)^\circ,\]
...
\begin{rem}[Nilpotent Lie algebras]\label{rem nilpotent Lie alg}
    A Lie algebra is called \emph{nilpotent}\index{Nilpotent!Lie algebra}\index{Nilpotent!Lie group} if there exists an $n\in\bbN$ such that $\ad_{A_1}\ad_{A_2}\cdots \ad_{A_n}=0$ for any sequence $A_1,\ldots,A_n\in\frakg$. In other words, the so called \emph{lower central series} of subalgebras \index{Central series}
    \[\frakg>[\frakg,\frakg]>[\frakg,[\frakg,\frakg]]>[\frakg,[\frakg,[\frakg,\frakg]]]>\cdots\]
...
\begin{proof}
    ...
    \begin{multline}
        \left|k\ln h_n\right|=\frac{k}{\lfloor t_n\rfloor}|\lfloor t_n\rfloor \ln h_n|\leq 
    \frac{k}{\lfloor t_n\rfloor}\left(|A|+\left|\lfloor t_n\rfloor \ln h_n-A\right|\right)\leq \\
    \leq \epsilon+\left|\lfloor t_n\rfloor \ln h_n-A\right|<\delta,
    \end{multline}
...
\begin{cor}[Closed Subgroup Theorem {{\cite[Cor.~1.10.7]{DK}}}]\label{cor 1.10.7 DK}\index{Theorem!Closed Subgroup (Cartan)}
...
\begin{lem}[{\cite[Lem.~9.2.26]{HN}}]\label{lem 9.2.26 HN}
...
\begin{cor}[{\cite[Cor.~9.5.11]{HN}}]\label{cor 9.5.11 HN}
...
\begin{thm}[Yamabe (1950) {{\cite[Thm.~9.6.1]{HN}}}]\label{thm Yamabe}\index{Theorem!Yamabe}
    A subgroup $H$ of a connected Lie group $G$ is path-connected iff it is an analytic subgroup, i.e., it is of the form $\<\exp_G\frakh\>$ for the Lie subalgebra $\frakh$ of $\Lief G$ determined by \index{Analytic subgroup}
...
\begin{example}[Siegel upper half-space]\index{Siegel upper half-space}\index{Hyperbolic half-plane}\label{ex siegel upper half-space}
    ...
    \[\calH_g \coloneqq \{Z=X+\i Y\mid X,Y\in \Mat_g(\bbR), X=X^\sfT, Y=Y^\sfT, Y>0\}.\]
    The symplectic group $\Sp_{2g}(\bbR)$ consists of real $2g\times 2g$ matrices of the block form 
    \[S=\begin{pmatrix}
        A & B\\
        C & D
    \end{pmatrix}\in\Sp_{2g}(\bbR),\quad A^\sfT C,B^\sfT D\text{ symmetric}, A^\sfT D-C^\sfT B=\rmI_g,\]
    and acts on $\calH_g$ by \glspl{flt}:
    \[S\cdot Z\coloneqq (AZ+B)(CZ+D)^{-1}.\]
    First one checks two identities:
    \begin{align}
        (AZ+B)^\sfT  (CZ+D)-(CZ+D)^\sfT (AZ+B)=Z-Z^\sfT &=0 \label{1y94}\\
        (AZ+B)^\sfT  (\widebar{CZ+D})-(CZ+D)^\sfT (\widebar{AZ+B})=Z-\wb{Z}&=2\i Y.\label{13702}
    \end{align}
    Consider a vector $\bf{z}\in\bbC^g$ such that $(CZ+D)\bf{z}=0$. Then 
    \[0=\bf{z}^\sfT  (AZ+B)^\sfT (\widebar{CZ+D})\wb{\bf{z}}=\bf{z}^\sfT  (CZ+D)^\sfT (\widebar{AZ+B})\wb{\bf{z}}\]
    and therefore $2\i \bf{z}^\sfT  Y \wb{\bf{z}}=0$. But $Y$ is positive definite, so $\bf{z}=0$. Thus $\ker(CZ+D)=\{0\}$ and the action is well-defined.
    ...
    \[\Im (S\cdot Z)=((CZ+D)^\sfT )^{-1}Y(\widebar{CZ+D})^{-1},\]
    and it is indeed positive definite if $Y$ is. Finally, we need to establish transitivity. For this one uses the Cholesky decomposition for positive definite symmetric matrices: there exists $U\in\GL_g(\bbR)$ such that $Y=U^\sfT U$, and hence $Z$ can be written as the following symplectic matrix acting on $\i \rmI_g\in \calH_g$:
    \[Z=(\i U^\sfT +XU^{-1})U^{-1}=\begin{pmatrix}
        \rmI_g & X\\
        0 & \rmI_g
    \end{pmatrix}\begin{pmatrix}
        U^\sfT  & 0\\
...
\begin{cor}[Structure of connected abelian Lie groups]\label{cor structure of abelian Lie groups}
...
\section{Semidirect products II}\label{sec: semidirect prod}
...
\begin{defn}[Extension of groups]
...
\begin{thm}[Open Mapping Theorem {\cite[Thm.~11.1.8]{HN}}]\label{thm 11.1.8 HN}
...
\begin{prop}[Characterization of semidirect products {\cite[Prop.~11.1.18]{HN}}]\label{prop 11.1.18 HN}
...
\begin{prop}[{\cite[Prop.~11.1.19]{HN}}]\label{prop 11.1.19 HN}
...
% \begin{thm}[Smooth Splitting Theorem {\cite[Thm.~11.2.21]{HN}}]\label{thm 11.2.21 HN}
%    Let $G$ be a simply connected Lie group and let $N\sub G$ be a normal analytic subgroup. Then $N$ is closed and there exists a smooth section $\sigma:G\slash N\to G$ so that the map 
%    \[G\slash N\times N\to G,\quad (p,n)\mapsto \sigma(p)n\] 
%    is a diffeomorphism, but in general not an isomorphism, of Lie groups. In particular, the groups $N$ and $G\slash N$ are simply connected.
% \end{thm}
% \begin{proof}
%     TODO: Requires Levi's Theorem.
% \end{proof}
...
\begin{defn}\label{def isotropy type}
...
\begin{example}
    ...
    When the orbit space consists of many connected components, there is an additional nonlocal invariant given by the label of the component. For instance, the orbit space $M_N\slash \SO_n$, in addition to the Gram matrix and its functions, allows extra $\SO_n$-invariants that depend on (the signs of) the $n\times n$ determinants composed of $n$-tuples of vectors $(\bf{x}_{i_1},\ldots,\bf{x}_{i_n})$, $i_1,\ldots,i_n\in \{1,\ldots,N\}$. These signs are not all independent: the true number of connected components of $M_N\slash \SO_n$ grows only linearly with $N$. This nonlocal invariant distinguishes orbits by \emph{parity}, i.e., orbits that are mapped to each other under any fixed reflection of $\bbR^n$, say $P=\diag(1,1,\ldots,1,-1)\in\Or_n$, which is not an element of $\SO_n$. 
...
\begin{defn}[Tubular neighborhood, Slice]\index{Tubular neighborhood}\index{Slice (of a Lie group action)}
...
\begin{example}\index{Hopf bundle}\label{ex Hopf principal bundle}
...
\begin{rem}
   ...
   Then the conjugation $A\mapsto \tau A\tau^\sfT $ on $\calO_n$ takes the subgroup $\calO_k\times\calO_{n-k}$ to $\calO_{n-k}\times\calO_{k}$ and hence induces a diffeomorphism $\Gr_k(\bbK^n)\cong \Gr_{n-k}(\bbK^n)$. Therefore, for the purposes of studying the topology of these spaces, we can always assume $n\geq 2k$.
\end{rem}
...
\begin{defn}[Tautological vector bundles]\index{Tautological bundle}\label{def tautological bundles}
...
\begin{defn}[Projective space and sphere]
    ...
    \[P=\left\{\begin{pmatrix}
        A&\bf{0}_n\\
        \bf{x}^\sfT &\det A^{-1}\end{pmatrix}, A\in\GL_n(\bbR),\bf{x}\in\bbR^{n}\right\}.\]
...
\begin{defn}[Nilpotent, solvable Lie algebra]\index{Nilpotent!Lie algebra}\index{Solvable!Lie algebra}
...
\begin{example}\label{ex 5.2.20 HN}
...
\begin{example}[Oscillator algebra]\index{Oscillator algebra}
...
\begin{defn}[Nilpotent, solvable Lie group]\index{Nilpotent!Lie group}\index{Sovable!Lie group}
...
\end{prop}
% \begin{proof}
    % TODO: requires the Smooth Splitting Theorem (11.1.21 HN), which requires Levi's Theorem.

    %The proof is broken down into two lemmas.

    % \begin{lem}[{\cite[Lem.~11.2.2]{HN}}]\label{lem 11.2.2 HN} 
    %     If $H,K< G$ are analytic subgroups with Lie algebras $\frakh,\frakk$, then $[H,K]$ is also an analytic subgroup and its Lie algebra contains $[\frakh,\frakk]$.
    % \end{lem}
    % \begin{proof}
    %     Since $H,K$ are path-connected, the set $S\coloneqq \{hkh^{-1}k^{-1}\mid h\in H,\; k\in K\}$ is path-connected, as a continuous image of $H\times K$. This implies that the set $T=S\cup S^{-1}$ is path-connected because $e\in S\cap S^{-1}$. We further conclude that all Cartesian products $T\times\cdots \times T$ are path-connected, and therefore the subgroup $[H,K]=\bigcup_m T^m$ is path-connected because it is an increasing union of path-connected subsets. Now Yamabe's Theorem~\ref{thm Yamabe} implies that the path-connected subgroup $[H,K]$ is analytic. Furthermore, the Initial Subgroup Theorem~\ref{thm initial subgroup} provides on $[H,K]$ the structure of a weakly embedded Lie subgroup with Lie algebra 
    %     \[\frakh=\{A\in\frakg:\; \exp_G(\bbR A)\subset [H,K]\}.\]
    %     To see that $[\frakh,\frakk]\subset \frakh$, let $A\in\frakh$, $B\in\frakk$. Then the path $\gamma:\bbR\to[H,K]$ defined by 
    %     \[\gamma(t)\coloneqq [\exp_G A,\exp_G tB]=\exp_G(A)\exp_G \left(-\rme^{t\ad_B}A\right)\]
    %     is a smooth path in $[H,K]$ with $\gamma(0)=e$, so that formula (\ref{dexp series}) for the logarithmic derivative of the exponential map leads to
    %     \[\frakh\ni \dot\gamma(0)=(\delta\gamma)(0)=\delta(\exp_G)_{-A}(-[B,A])=\left(\rme^{\ad_A}-\rmI\right)B.\]
    %     We conclude that, for each $s\in\bbR$, $\rme^{s\ad_A}B-B\in\frakh$, and by taking the derivative at $s=0$, we obtain $[A,B]\in\frakh$.
    % \end{proof}
    % \begin{lem}[{\cite[Prop.~11.2.3]{HN}}]
    %     Let $H,K,L$ be analytic subgroups of a connected Lie group $G$ with Lie algebras $\frakh,\frakk,\frakl$, satisfying 
    %     \[[\frakh,\frakl]\subset\frakl,\quad [\frakk,\frakl]\subset\frakl,\quad [\frakh,\frakk]\subset\frakl.\]
    %     Then $[H,K]\subset L$. If, in addition, $[\frakh,\frakk]=\frakl$, then $[H,K]=L$.
    % \end{lem}
    % \begin{proof}
    %     From our assumptions, it immediately follows that $\frakh+\frakk+\frakl$ is a subalgebra of $\frakg$, and we may assume that it coincides with $\frakg$. Then $\frakl$ is an ideal of $\frakl$. First, we assume that $G$ is simply connected. Now $L$ is a closed subgroup of $G$ by Theorem~\ref{thm 11.2.21 HN}, so that $G\slash K$ carries a natural Lie group structure. Let $q:G\to G\slash L$ be the quotient morphism with $\ker q=L$. The relation $\ker q_{\ast e}=\frakl$ yields 
    % \[[q_{\ast e}\frakh,q_{\ast e}\frakk]\subset q_{\ast e}[\frakh,\frakk]=\{0\}.\]
    % Therefore, the two subalgebras $q_{\ast e}\frakh$ and $q_{\ast e}\frakk$ commute, which implies that the corresponding analytic subgroups $q(H)=\<\exp_{G\slash L}q_{\ast e}\frakh\>$ and $q(K)=\<\exp_{G\slash L}q_{\ast e}\frakk\>$ commute. This implies that $[H,K]\subset\keq q=L$.

    % Now let $G$ be arbitrary, $q_G:\wt G\to G$ be the universal covering morphism of $G$, and $\wt H,\wt K$, and $\wt L$, resp., the analytic subgroups of $\wt G$ corresponding to $\frakh,\frakk$, and $\frakl$. By what we have just seen, $[\wt H,\wt K]\subset \wt L$, and this implies that 
    % \[[H,K]=q_G\left((\wt H,\wt K)\right)\subset q_G(\wt L)=L.\]
    
    % If, in addition, $[\frakh,\frakk]=\frakl$, then Lemma~\ref{lem 11.2.2 HN} implies that $\frakl=[\frakh,\frakk]\subset \Lie([H,K])$, so that the analytic subgroups $[H,K]$ and $L$ must coincide.
    % \end{proof}
% \end{proof}
...
\begin{prop}[{\cite[Prop.~5.2.3]{HN}}]\label{prop 5.2.3 HN}
...
\begin{lem}[{\cite[Lem.~5.2.4]{HN}}]\label{lem 5.2.4 HN}
    \begin{enumerate}[label=(\roman*)]
...
\begin{thm}[Engel {\cite[Thm.~5.2.5]{HN}}]\label{thm 5.2.5 HN}
...
Notably, Engel's Theorem does not hold on infinite-dimensional spaces. From it, one also gets the induction step by which one shows that every subalgebra of $\frakgl(V)$ consisting of nilpotent elements can be written as a Lie algebra of triangular matrices.

\begin{defn}[Complete flag]\index{Flag!Complete}
...
\begin{thm}[Engel {\cite[Thm.~5.2.8]{HN}}]\label{cor 5.2.8 HN Engel}
...
\begin{defn}[Nilradical]\index{Nilradical}
...
\begin{thm}[{\cite[Thm.~11.2.6]{HN}}]\label{thm 11.2.6 HN}
...
We are ready to extend the description of the structure of connected abelian Lie groups (Corollary~\ref{cor structure of abelian Lie groups}) to connected nilpotent Lie groups.

\begin{thm}[Structure of connected Nilpotent Lie groups {\cite[Thm.~11.2.10]{HN}}]
...
\begin{prop}[{\cite[Prop.~5.4.3]{HN}}]\label{prop 5.4.3 HN}
...
\begin{prop}[{\cite[Prop.~1.23]{Knapp}}]\label{prop 1.23 Knapp}
    ...
    such that each is an ideal inside the next, $\frakg_i\sub\frakg_{i+1}$, $i=0,\ldots,n-1$. Such a sequence is called an \emph{elementary sequence}.\index{Elementary sequence}
\end{prop}
\begin{proof}
...
\begin{defn}[Radical]\index{Radical}
...
\begin{thm}[{\cite[Thm.~5.4.7]{HN}}]\label{thm 5.4.7 HN}
...
\begin{thm}[Lie {\cite[Thm.~5.4.8]{HN}}]\label{thm 5.4.8 HN Lie}\label{Theorem!Lie}
...
\begin{defn}[Real form of a Lie algebra]\index{Real form!of a Lie algebra}
...
\begin{example}[Non-isomorphic real forms of $\fraksl_2(\bbC)$]
...
\begin{example}[Lie algebra with no real form]
...
\begin{rem}[Complexification of Lie groups]
...
\begin{defn}[Nilpotent representation]
...
\begin{cor}[{\cite[Cor.~5.4.11]{HN}}]\label{cor 5.4.11 HN}
...
% The structure of solvable Lie groups is substantially more complicated than that of the nilpotent ones. In particular, the exponential map need not be surjective. A simple example with this property is the universal covering group of the Euclidean group $\SE_2$ of the plane. Other difficulties aries from the fact that the center of a connected solvable Lie group is not connected (again $\SE_2$ provides an example). Therefore, the description of the center of the universal covering group and hence the classification of all solvable Lie groups with a given Lie algebra is more complicated than the nilpotent case, where everything boils down to factorization by a discrete subgroup of $\frakz(\frakg)$. Presently, we can only show that connected and simply connected solvable Lie groups are diffeomorphic to vector spaces by showing the existence of global coordinates of ``the second kind'' (products of exponentials).\footnote{Let $\bf{t}_i$, $i=1,\ldots,n$, be a basis of the Lie algebra $\frakg$. Coordinates of the first kind on the Lie group $G$ have the form $\bf{x}\mapsto \exp_G(x^i\bf{t}_i)$, whereas coordinates of the second kind are $\bf{x}\mapsto \exp_G(x^1\bf{t}_1)\cdots\exp_G(x^n\bf{t}_n)$.}

% \begin{thm}[{\cite[Thm.~11.2.14]{HN}}]
%     If $G$ is a connected and simply connected solvable Lie group, then there exists a basis $\bf{t}_1,\ldots,\bf{t}_n$ of its Lie algebra $\frakg$ such that the map 
%     \[\varPhi:\bbR^n\to G,\quad (x^1,\ldots,x^n)\mapsto \prod_{j=1}^n \exp_G(x^j\bf{t}_j)\]
%     is a diffeomorphism, the \glspl{ops} $R_j\coloneqq \exp_G(\bbR\bf{t}_j)$ of $G$ are closed, and 
%     \[G\cong \left(\left(\ldots \left((R_1\rtimes R_2)\rtimes R_3\right)\ldots \right)\rtimes R_n\right).\]
% \end{thm}
% \begin{proof}
    % TODO: once again this uses the Smooth Splitting Theorem, which requires Levi's Theorem.
% \end{proof}
...
\begin{prop}\label{prop lambda-equiv maps}
...
% and have the transformation rule \eqref{eq transformation of Phiab} take the following form:
...
\begin{example}\label{ex associated bundles}
    \begin{enumerate}
        ...
        called the \emph{adjoint group bundle} of $P$. \index{Adjoint group bundle}

        \item If $P$ is a principal $G$-bundle and $\frakg=\Lief G$ with $G\overset{\Ad}{\acts}\frakg$ being the adjoint representation, then the associated bundle \[\Ad(P)\coloneqq P^{[\Ad]}=P\times^G \frakg\] is called the \emph{adjoint vector bundle} of $P$. \index{Adjoint vector bundle}
        ...
        \item Using the same idea for a real \gls{vb} $E$ and $s\in\bbR$, the line bundles of \emph{scalar $s$-densities}\index{Density!Scalar $s$-} can be defined as
        \[\left|\bigwedge\right|^s E\coloneqq \Fr(E)^{\left[|\det \sigma|^{-s}\right]}=\Fr(E)\times^{\GL_k(\bbR)}\bbR,\]
        where the action $|\det \sigma|^{-s}$ multiplies an element of $\bbR$ by the $(-s)$-th power of the absolute value of the determinant of the element of $\GL_k(\bbR)$. In particular, sections of $\left|\bigwedge\right|^{-1/2}E$ are called \emph{half-densities}.\index{Density!Half-}\index{Density!bundle}
...
\begin{example}[$r$-th order frame bundle]\label{ex r-th frame bundle}
    Recall Definition~\ref{def jet bundles} of jet bundles. We can naturally identify the set $\rmJ^1_0(\bbR^n,\bbR^n)_0^\times$ of invertible $1$-jets on $\bbR^n$ with source and target zero with the group $\GL_n(\bbR)$. This leads to an interpretation of the linear frame bundle $\Fr(\T M)$ of an $n$-dimensional manifold $M$ as the space $\rmJ^1_0(\bbR^n,M)^\times$ of invertible $1$-jets from $\bbR^n$ to $M$ with source $0$.\index{Frame bundle!Higher-order}
...
\begin{thm}[{{\cite[Thm.~1.2.8]{Cap}}}]\label{thm 1.2.8 Cap}
...
\begin{rem}
    \begin{enumerate}
        \item It might seem strange that we have identified a group, which has a canonical basepoint, with spaces of sections, because most bundles don't have any distinguished sections. However, since the structure group of the group bundle $P^{[\Adg]}$ acts by conjugation, it preserves the identity of the typical fiber $G$, and thus this bundle does have a natural \emph{identity section} (cf.\ Example~\ref{ex non-vector G-bundles}). \index{Identity section} Similarly, $P\times^G_MP$ has the same kind of section given by $m\mapsto [(p,p)]$ where $p\in \pi^{-1}(m)$.
...
\begin{rem}
    \begin{enumerate}
        ...
        \item In the language of gauge theories, bundle reduction represents \emph{symmetry breaking}:\index{Symmetry breaking} from the whole (unbroken) symmetry group $G$ to the group $H$ of reduced symmetry. A section $h$ of $P\slash H$ can be interpreted as a \emph{classical Nambu-Goldstone field} (see \cite{Radosevic} and also ``Higgs field'' in \cite{Giachetta}).\index{Nambu-Goldstone field} Let us denote the $H$-subbundle corresponding to $h$ by $\prescript{h}{}{P}<P$ as above. Matter fields are sections of (typically vector) bundles associated to $\prescript{h}{}{P}$, so they transform under some representation of $H$. For example, ``$V$-valued matter fields'' are sections of $E=\prescript{h}{}{P}\times^H V$ for some representation $H\acts V$. If we choose another field $h'$, we get an isomorphic, but not canonically isomorphic, $H$-bundle $\prescript{h'}{}{P}$ and associated bundle $\prescript{h'}{}{P}\times^H V$. Thus, matter fields associated to one Nambu-Goldstone field cannot, in general, be naturally interpreted as being associated to another one. In other words, matter fields must be described as \emph{pairs} $(h,s)$, where $s$ is a section of $\prescript{h}{}{P}\times^H V$. An isomorphism by which a general \gls{fb} is identified with an associated bundle of a given principal bundle is called a \emph{soldering}. Thus, a choice of Nambu-Goldstone field is a choice of soldering that allows one to interpret sections of a given bundle $E$ as matter fields associated to a particular $H$-structure $\prescript{h}{}{P}$. The interplay between soldering and $G$-structures will be examined in depth in \Chap~\ref{ch affine structures}.
    \end{enumerate}
\end{rem}
...
\begin{defn}[Geometric structure]\index{Geometric structure}\label{def geom structure}
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item An $\{e\}$-structure is equivalent to a global section of $P$, a global trivialization of $P$, or an equivariant $G$-valued function on $P$ (with the left action $G\overset{\rmL}{\acts}G$ on the target). $\{e\}$-structures are also called \emph{absolute parallelisms}.\index{Absolute parallelism}
    \end{enumerate}
