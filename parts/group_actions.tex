...
\begin{rem}[Nilpotent Lie algebras]\label{rem nilpotent Lie alg}
    ...
    Nilpotent Lie algebras and groups are a very important class because all of the group structure is polynomial, making it ``almost abelian''. See \S\ref{sec: nilpotent groups} for more on them. The next simplest class of Lie groups are solvable Lie groups, which we will study later in the book.
\end{rem}
...
In this section we study the basic structure of nilpotent (and, to much lesser extent, solvable) Lie algebras and groups. First we will need a criterion for a Lie group to decompose as a semidirect product, to which the first part of this \sect\ is devoted.

\begin{defn}[Extension of Lie groups]
    An extension of Lie groups is a short exact sequence
    \[\{e\}\to N\overset{i}{\hookrightarrow} G\overset{q}{\to} H\to \{e\}\]
    of Lie group homomorphisms, i.e., such that $i:N\to \ker q$ is an isomorphism.
\end{defn}

A particularly important class of extensions of a Lie group $H$ by a Lie group $N$ are the semidirect products $H\ltimes_\alpha N$. We start with a criterion for a Lie group to decompose as a semidirect product. First we note that, by analogy with semidirect products of Lie groups, one can define \emph{semidirect sums} of Lie algebras $\frakg\ltimes_\beta\frakn$, where $\beta:\frakg\to \Der(\frakn)$ is a Lie algebra homomorphism which defines an action of $\frakg$ on $\frakn$ by derivations (infinitesimal automorphisms).\index{Semidirect sum of Lie algebras} The Lie bracket on the semidirect sum is defined as 
...
\begin{prop}[{\cite[Prop.~11.1.19]{HN}}]\label{prop 11.1.19 HN}
    Let $G$, $H$, and $N$ be simply connected Lie groups with Lie algebras $\frakh,\frakh$, and $\frakn$. Suppose that $\frakg\cong\frakh\ltimes_\beta\frakn$ is a semidirect sum for some morphism $\beta:\frakh\to \Der(\frakn)$.\footnote{Recall that $\Der(\frakn)<\frakgl(\frakn)$ is the Lie algebra of the full automorphism group $\Aut(\frakn)<\GL(\frakn)$.} Then there is a unique smooth action $\gamma:H\to \Aut(N)\cong \Aut(\frakn)$ with $\gamma_{\ast e}=\beta:\frakh\to \Der(\frakn)$ and the two natural homomorphisms $i_H:H\to G$, $i_N:N\to G$ combine to an isomorphism 
...
\begin{rem}
    If we apply Lie's Theorem~\ref{thm 5.4.8 HN Lie} to $V=\frakg$ and $\ad(\frakg)$, where $\frakg$ is solvable, we get a complete flag of ideals 
    \[0=\frakg_0<\frakg_1<\cdots<\frakg_n=\frakg\]
    of $\frakg$ with $\dim\frakg_k=k$. Such a chain is called a \emph{H\"older series} for $\frakg$.
\end{rem}
