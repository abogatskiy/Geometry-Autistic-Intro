...
\begin{thm}[{{\cite[Thm.~8.7]{Sharpe}}}]\label{thm 8.7 Sharpe}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item The period group $\varGamma=\Phi_\eta(\pi_1(M,x_0))\subset G$ acts by left multiplication on $G$ as the group of deck transformations for $\pi$.
    \end{enumerate}
\end{thm}
...
\begin{xca}
    \begin{enumerate}
        \item Describe the \emph{second tanget bundle} $\T (\T M)\to M$ as a natural bundle, i.e., as associated to $\Fr^2(\T M)$ with some typical fiber. Compare to the second-order tangent bundle $\T^2 M=\rmJ^2_0(\bbR,M)$.
...
\begin{defn}[$G$-structure]\index{$G$-structure}
    ...
    A \emph{$G$-structure of order $r$ on a manifold $M$} is a $G$-structure on a local natural bundle $\bf{F}(M)$ of order $r$, or, equivalently, a bundle reduction of the $r$-th order frame bundle $\Fr^r(\T M)$.
\end{defn}
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item An $\{e\}$-structure is equivalent to a global section of $P$, a global trivialization of $P$, or an equivariant $G$-valued function on $P$ (with the left action $G\overset{\rmL}G$ on the target). $\{e\}$-structures are also called \emph{absolute parallelisms}.\index{Absolute parallelism}
    \end{enumerate}
