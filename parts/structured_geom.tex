...
\begin{defn}[Geodisic]\index{Geodisic}
...
We say that an affine connection $\calH M$ on $M$ is complete if every geodisic may be extended to $I_\epsilon=\bbR$.
...
\begin{prop}[{{\cite[p.~249]{Spivak2}}}]\label{prop geodisics coincide}
...
\begin{defn}[Geodisically convex sets]
...
\begin{thm}\label{geodesically convex nbhds thm}
    On a Riemannian manifold, geodisically convex neighborhoods:
    \begin{enumerate}
