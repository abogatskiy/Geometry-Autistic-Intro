...
\begin{defn}[Effective $(G,H)$-structure]
...
\subsection{Rigidity of submanifolds}
...
\begin{example}[Curves in $\bbE^n$]\label{ex serret-frenet}
    ...
    In particular, in three dimensions, $\tau\coloneqq \kappa_2$ is called the \emph{torsion}\index{Torsion!of a curve} of $\gamma$. The formula for the logarithmic derivative of the frame $\frake:\bbR\to \SO_n$ is given by the \emph{Frenet-Serret equation}\index{Equation!Frenet-Serret}
...
\begin{example}[Holomorphic maps and the M\"obius group {{\cite[\S1.7]{Ivey}}}]
    ...
    \]
    The group of \glspl{flt} is the M\"obius group\index{M\"obius group} $\PSL_2(\bbC)=\SL_2(\bbC)\slash \{\pm I\}$ (cf.\ Example~\ref{ex Lie group representations}), which acts transitively on $\CP^1$.  Thus, $\CP^1$ is a homogeneous space $\PSL_2(\bbC)\slash P$, where 
    \[P=\left[
        \begin{pmatrix}
            a & b \\
            0 & a^{-1}
    ...
    is a differential invariant called the \emph{Schwarzian derivative}.\index{Schwarzian derivative}
\end{example}
...
Since $Q$ is a quadratic form, it can be diagonalized, giving rise to the following important data.
...
\]
where $h_{ij}=h_{ji}$ are some uniquely determined functions defined on $P$ (they are related to the Hessian of the parametrizing immersion $\bf{x}:U\to \bbE^2$). Given an adapted (local) lift $\frake:\Sigma\to P$, we have 
...
Having constructed $h$, we can finally define a natural orthonormal frame. Note that when the eigenvalues $k_1,k_2$ coincide, they may not be differentiable functions of the point of $\Sigma$. Such points are called \emph{umbilic}. Away from umbilic points, $k_1$ and $k_2$ are smooth functions, and we may further adapt the frames by diagonalizing $h$.
...
Nevertheless, $H$ and $K$ are usually sufficient to determine $\Sigma$ up to congruence. Those surfaces for which this is not the case either have constant $H$ or belong to a finite-dimensional famile called \emph{Bonnet surfaces}.
...
Let us now derive the compatibility condition on $h$ by setting up a Pfaffian system. Treat $\{h_{11},h_{12},h_{22}\}$ as coordinates in $\rmS^2 \bbR^2\cong\bbR^3$ and let $J\subset \Omega^{\smbullet }(P\times \SE_3\times \bbR^3)$ be generated as follows:
\[J\coloneqq \<\theta^3,\omega_1^3-h_{11}\theta^1-h_{12}\theta^2,\omega_2^3-h_{12}\theta^1-h_{22}\theta^2\>_{\mathrm{diff}}.\]
We know that along the hypothetical surface $\Sigma$, these three forms must vanish. We also have the independence condition $\theta^1\wedge\theta^2$, meaning that this differential form must be nowhere-vanishing after pullback to $\Sigma$. The integral manifolds of this system are graphs of immersions $\restr{P}{U}\to \SE_3$ that are adapted frame bundles of isometric immersions $\bf{x}:U\to \bbE^3$ satisfying $j^\ast \rmI=\sfg$ and $j^\ast Q=h$. By the Frobenius Theorem~\ref{thm 4.7.8 RS1}, the surface $\Sigma$ (locally) exists iff $J$ is an \gls{eds}. First, it is easy to check that $\dd\theta^3\equiv 0 \pmod{\Delta}$. The differentials of the other two forms can be combined into a column-vector, giving the following integrability condition:
\begin{multline}
    \dd \left\{
        \begin{pmatrix}
            \omega_1^3\\\omega_2^3
...
Since it is not obvious whether a given \gls{eds} is a Pfaffian system or not (i.e.\ whether it can be generated by $1$-forms), we can restate the Frobenius Theorem~\ref{sec: frobenius ii} in the form of a criterion on a general \glspl{eds}.
...
\begin{example}
    The equation $u_{xy}=0$ may be encoded as a tableau of order two for $V=\bbR^2$, $W=\bbR$ as 
    \[A=\{p_{11}\dd x^1\dd x^1+p_{22}\dd x^2\dd x^2\mid p_{11},p_{22}\in\bbR\}.\]
\end{example}
...
\begin{proof}
    ...
    \[\dim A_{k-1}{}^{(1)}-\dim A_[k^{(1)}]\leq \dim A_{k-1}.\]   
...
We also specify $n$ $1$-forms $\{\omega^1,\ldots,\omega^n\}$,  which induce our independence condition $\omega^1\wedge\cdots\wedge\omega^n$. We describe this problem by the pair 
\[(I,J),\quad J=\<I\cup\{\omega^i\}_{i=1}^n\>.\]
...
\begin{example}[Linear Weingarten surfaces]
...
\begin{example}[Liouville equation]\index{Equation!Liouville}
    Surfaces with given Gauss curvature have an especially simple description in isothermal coordinates. If the metric (i.e.\ First Fundamental Form) is $\sfg=\lambda^2(\dd x^2+\dd y^2)$, then 
    ...
    In particular, when $K$ is constant, this equation for the unknown conformal factor $\varphi\coloneqq \ln\lambda$ is called Liouville's equation:
    \[\Delta \varphi+2K\rme^{\varphi}=0,\]
...
\begin{example}[Minimal surfaces with given curvature]
    ...
    \begin{align}
        \dd\pi =&d(-2k\omega^2_1+k_2\theta^1-k_1\theta^2)\\
...
\begin{defn}[Retracting system of $\calI$]
    Let $\calI$ be an \gls{eds} on $M$ with characteristic distribution $\calA(\calI)$. The restracting space, or Cartan system, of $\calI$ is the Pfaffian system $\calC(\calI)<T^\ast M$ defined by 
...
\begin{example}[Example~\ref{ex 7.1.10 Ivey} continued]
...
\subsection{Hyperbolic second-order EDS}
...
\begin{example}[Liouville equation]\label{ex 7.3.6 Ivey}
    For Liouville's equation $z_{xy}=\rme^z$, the Pfaffian system $I$ is defined by \index{Equation!Liouville}
    \[
    \begin{matrix}
...
\begin{defn}[Darboux-integrable system]\index{Darboux integrability}
    A hyperbolic \gls{eds} $\calI$ is said to be Darboux-integrable if each characteristic system $\calM_i$ contains a Frobenius system $\Delta_i$ of rank two that is independent from $I$.
\end{defn}
...
\subsection{Monge-Amp\`ere systems and Weingarten surfaces}
