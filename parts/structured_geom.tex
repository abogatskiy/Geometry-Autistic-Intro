...
\begin{example}
    \begin{enumerate}
        \item $(\Aff_n(\bbK), \GL_n(\bbK))$ is the affine geometry on $\bbK^n$, and the bundle $\Aff_n(\bbK)\to \rmA(\bbK^n)$ is the frome bundle of the affine space $\rmA(\bbK^n)$. Similarly, $(\Aff_n^+(\bbK), \GL_n^+(\bbK))$ is the affine geometry with an orientation.
...
\begin{defn}[Isotropy representation]
    For a Klein geometry $(G,H)$, we call the representation $\underline{\Ad}:H\to \GL(\frakg\slash\frakh)$ given by $h\mapsto \wh{L}_{h\ast [e]}$, is called the isotropy representation of the Klein geometry.
...
\begin{example}[Curves in $\bbE^n$]
    ...
    \[\left(\frac{\dd}{\dd s}\gamma,\frac{\dd^2}{\dd s^2}\gamma,\ldots,\frac{\dd^n}{\dd s^n}\gamma\right),\] assume that they are linearly independent (such curves are called nondegenerate) and apply the Gram-Schmidt process to them to get $\{e_1,\ldots,e_{n-1}\}$ (note that the Gram-Schmidt process doesn't change the differential order of these vectors). The final vector is then uniquely defined as $e_n=e_1\times\cdots\times e_{n-1}$. Now we define the \emph{generalized curvatures}
    \[\kappa_i\coloneqq -\<\frac{\dd}{\dd s}e_i,e_{i+1}\>,\quad i=1,\ldots,n-1.\] 
    The formula for the logarithmic derivative of the frame $\frake:\bbR\to \SO_n$ is given by the \emph{Frenet-Serret equation}\index{Equation!Frenet-Serret}
    \[\frac{\dd}{\dd s}\frake=\begin{pmatrix}
        0 & -\kappa_1 & &\\
        \kappa_1 &\ddots & \ddots&\\
         & \ddots &0 &-\kappa_{n-1}\\
         0 & &\kappa_{n-1} & 0
    \end{pmatrix}\cdot \frake.\]
    Therefore, nondegenerate curves in $\bbE^n$ are determined by their generalized curvatures $\{\kappa_1,\ldots,\kappa_{n-1}\}$ up to a rigid motion. We say that the generalized curvatures provide a \emph{complete set of differential invariants}.
\end{example}
...
\begin{proof}
    ...
    so $x\mapsto \sigma_x$ is $G$-equivariant. Since for any $G$-equivariant map $x\mapsto \sigma_x$ we must have $\sigma_x(gH)=g\cdot g^{-1}\cdot \sigma_x(g^{-1}gH)=g\cdot \sigma_{g\cdot x}([e])$, the two constructions are inverse bijections between the set of $G$-equivariant maps $X\to \Gamma^\infty(E)$ and the set of $H$-equivariant maps $X\to E_{[e]}$.

    Taking $X=\{*\}$ a single point with the trivial $G$-action, a $G$-equivariant map $X\to \Gamma^\infty(E)$ is just a $G$-invariant section $g\cdot \sigma_*=\sigma_*$, and a $H$-equivariant map $X\to E_{[e]}$ is an $H$-invariant element in $E_{[e]}$. Thus we get a bijection $\Gamma^\infty(E)^G\to (E_{[e]})^H$.
...
Note that $\theta_U$ \emph{almost} determines $\sigma$ since the Fundamental Theorem~\ref{thm 6.1 Sharpe fundamental local} says that $\theta_U$ determines $\sigma$ up to left multiplication by a fixed element of $G$. Indeed, for an effective Klein geometry, if $\sigma$ is a section, then $g\sigma$ is a section only if $g=e$.\footnote{If $g\sigma$ is a section, then $u=\pi(g\sigma(u))$ and hence $g\cdot u=u$ for all $u\in U$. Thus $f^{-1}gf\in H$ for all $f\in\pi^{-1}(U)\subset G$. Thus shows that $\{f\in G\mid f^{-1}gf\in H\}$ contains an open subset of $G$. But it is also an analytic subset of $G$, so it must be equal to $G$, at least for connected $G$. Thus $g\in \bigcap_{f\in G}fHf^{-1}$, and this latter is a normal subgroup of $G$ inside $H$ and is therefore trivial.} 
...
\subsection{Theory of surfaces}\label{sec: theory of surfaces}

In this section we will describe a classical application of the Frobenius Theorem in the context of Gauss' theory of surfaces. 



Let $\Sigma$ be an oriented $2$-dimensional manifold and let $i:\Sigma\to \bbR^3$ be an immersion. We will view $\Sigma$ as a subset of $\bbR^3$, i.e.\ a \emph{surface}, and we will assume the standard metric on $\bbR^3$. The dot product of vectors will be denoted by $\<\,,\,\>$. This dot product can be restricted to tangent vectors of $\Sigma$ to produce a metric on $T\Sigma$ that has a special name for historic reasons.

\begin{defn}[First fundamental form]
    The first fundamental form of a surface $\Sigma\subset\bbR^3$ is the quandratic form $\rmI\in\Gamma^0_2(\Sigma)$ defined by 
    \[\rmI_p(X,Y)=\<X,Y\>.\]
\end{defn}
...
Thus, let $e_3(p)$ for each $p\in\Sigma$ be the unit normal to $\Sigma$, pointing outward in the sense of Theorem~\ref{thm orientation via normal vector}.  If $\Sigma$ is locally parametrized by a function $\bm{x}: \bbR^2\supset U\to\bbR^3$ of two real parameters $u,v\in U$, then 
\[e_3=\frac{\bm{x}_u\times \bm{x}_v}{\lVert \bm{x}_u\times \bm{x}_v\rVert},\]
where $\bm{x}_u=\partial_u \bm{x}$ and $\bm{x}_v=\partial_v \bm{x}$. It is easy to verify that this vector is independent of the specific parametrization of $\Sigma$. Since $e_3$ is of unit length, we also conclude that $\partial_u e_3$ and $\partial_v e_3$ are tangent to $\Sigma$.
...
\begin{defn}[Gauss map]
    ...
    It is conventional to denote its components in an arbitrary local parametrization of $\Sigma$ by a pair of real variables as follows:
    \[(\rmI_{ij})=\begin{pmatrix}
        g_{11}&g_{12}\\
        g_{21}& g_{22}
...
The Gauss map is clearly differentiable and thus we have $(e_3)_\ast:T\Sigma\to T\bbS^2$. For a given point $p\in\Sigma$, notice that the tangent planes $T_p\Sigma$ and $T_{e_3(p)}\bbS^2$ are parallel and thus can be identified with each other by parallel transporting tangent vectors in $\bbR^3$. Therefore we obtain the equivalent map $(e_3)_{\ast p}:T_p\Sigma\to T_p\Sigma$, called the \emph{shape operator} of $\Sigma$.
...
\end{lem}
\begin{proof}
    Denote $N\coloneqq e_3$ for simplicity. Let $\bm{x}:\bbR^2\supset U\to \bbR^3$ be a local parametrization of $\Sigma$ at $p\in\Sigma$, and let $\{\bm{x}_u, \bm{x}_v\}$ be the corresponding basis of $T_p\Sigma$. It suffices to prove that $\<N_{\ast p}(\bm{x}_u),\bm{x}_v\>=\<\bm{x}_u,N_{\ast p}(\bm{x}_v)\>$. Let $\gamma(t)=\bm{x}(u(t),v(t))$ be a parametrized curve in $\Sigma$ with $\gamma(0)=p$. We have 
    \[N_{\ast p}\dot\gamma(0)=N_{\ast p}(\bm{x}_u \dot u(0)+\bm{x}_v \dot v(t))=\restr{\frac{\dd}{\dd t}}{t=0}N(u(t),v(t))=N_u \dot u(0)+N_v\dot v(0).\]
    In particular, $N_{\ast p}(\bm{x}_u)=N_u$ and $N_{\ast p}(\bm{x}_v)=N_v$. Therefore we need to prove that 
    \[\<N_u,\bm{x}_v\>=\<\bm{x}_u,N_v\>.\]
    To see this, take the derivatives of $\<N,\bm{x}_u\>=0$ and $\<N,\bm{x}_v\>=0$ relative to $u$ and $v$, repsectively, to obtain 
    \begin{align}
        \<N_v,\bm{x}_u\>+\<N,\bm{x}_{uv}\>&=0,\\
        \<N_u,\bm{x}_v\>+\<N,\bm{x}_{vu}\>&=0.
    \end{align}
    Thus, 
    \[\<N_u,\bm{x}_v\>=-\<N,\bm{x}_{uv}\>=\<N_v,\bm{x}_u\>.\]
\end{proof}

Therefore $(e_3)_\ast$ can be viewed as a quadratic form (symmetric tensor of type $(0,2)$) on $T\Sigma$.

\begin{defn}[Second fundamental form]
    The second fundamental form of a surface $\Sigma\subset\bbR^3$ is the quadratic form $\rmII\in\Gamma^0_2(\Sigma)$ defined by 
    \[\rmII_p(X,Y)=-\<(e_3)_{p\ast}X,Y\>.\]
    In local parametrization $\bm{x}(u,v)$, the components of $\rmII$ read 
    \[(\rmII_{ij})=\begin{pmatrix}
        \<e_3,\partial_u^2 \bm{x}\>& \<e_3,\partial_{uv}^2 \bm{x}\>\\
        \<e_3,\partial_{vu}^2 \bm{x}\> &\<e_3,\partial_v^2 \bm{x}\>
    \end{pmatrix}
    =\begin{pmatrix}
        L&M\\
        M&N
...
Since $\rmII$ is a quadratic form, it can be diagonalized, giving rise to the following important data.

\begin{defn}[Principal data]
    \begin{enumerate}
        \item The eigenvalues $k_1,k_2$ of the shape operator $N_\ast$ at $p\in \Sigma$ are called the \emph{principal curvatures}.
        \item The unit eigenvectors $v_1,v_2$ of $(e_3)_\ast$ are called the \emph{principal directions}.
        \item The \emph{Gaussian curvature} of $\Sigma$ is $K\coloneqq k_1k_2$. It also satisfies the \emph{Gauss equation}
        \[K=\frac{\det\rmII}{\det\rmI}=\frac{LN-M^2}{EG-F^2}.\]
        \item The \emph{mean curvature} of $\Sigma$ is $H\coloneqq \frac12(k_1+k_2)=\frac12\tr((\rmII)(\rmI)^{-1})$. Surfaces with identically vanishing mean curvature are called \emph{minimal}.
    \end{enumerate}
\end{defn}

Quantities on $\Sigma$ that depend only on the first fundamental form $\rmI$ are called intrinsic. Clearly $\rmII$ is extrinsic because it depends on the construction of a fiber bundle that depends on the embedding of $\Sigma$ into $\bbR^3$ (through the normal vector $e_3$). However, we can obtain intrinsic invariants from $\rmII$. Gauss' most famous result states that $K$ is an intrinsic differential invariant.
...
\[\SE_3=\SO_3\ltimes\bbR^3=\left\{g=\begin{pmatrix}
    R & \bm{t}\\
    0 & 1
\end{pmatrix}\middle| \bm{t}\in\bbR^3,R\in\SO_3
\right\}.\]
...
\theta_{\SE_3}=
    \begin{pmatrix}
        0 & -\omega_1^2 & -\omega_1^3& \omega^1\\
        \omega_1^2 & 0 & -\omega_2^3 & \omega^2\\
        \omega_1^3 & \omega_2^3 & 0 & \omega^3\\
        0 & 0& 0& 0
    \end{pmatrix},
\]
where $\omega^i,\omega^i_j\in\Omega^1(\SE_3)$ form a basis for the space of left-invariant $1$-forms on $\SE_3$. Since $\dd g=g\omega$, the last column implies 
\[\dd \bm{x}=e_i\omega^i.\]
Thus, $\omega^i$ has the geometric interpretation of measuring the infinitesimal motion of the point $\bm{x}$ in the direction of $e_i$. Similarly, $\omega^i_j$ measures the infinitesimal motion of $e_j$ toward $e_i$, i.e.\ rotations:
\[\dd e_j=e_i\omega^i_j.\]
It's obvious that $\omega^i$ are horizontal for the principal bundle $\pi:\SE_3\to\bbE^3$ since the right $\SO_3$-action changes only the rotational components of $g$.

The fact that the frame $\frake:\Sigma\to \SE_3$ is adapted means that, at every point of $\Sigma$,
\[\frake^\ast(e_3)=0,\quad \frake^\ast(\omega^1\wedge\omega^2)\neq 0.\label{eq 2.2 Ivey}\]
Let $\pi:P\to \Sigma$ be the bundle whose fiber over $p\in\Sigma$ is the set of oriented orthonormal bases $(e_1,e_2,e_3)$ of $T_p \bbE^3$ such that $e_3\perp T_{e_3(p)}\Sigma$. The first-order adapted lifts are exactly the sections of $P$. By fixing a frame at the origin in $\bbE^3$, $\SE_3$ is the total space of the bundle of all oriented orthonormal frames of $\bbE^3$, and $P$ is a subbundle of $\restr{\SE_3}{\Sigma}$.
...
\begin{lem}[Cartan]\label{lem cartan}
    Let $v_1,\ldots,v_k\in V$ be linearly independent elements of a vector space $V$ and let $w_1,\ldots,w_k\in V$ be such that 
    \[w_1\wedge v_1+\cdots w_k\wedge v_k=0.\]
    Then there exist scalars $h_{ij}=h_{ji}$, $1\leq i,j\leq k$, such that $w_i=\sum_j h_{ij}v_j$.
\end{lem}

Write $i_P:P\hookrightarrow \SE_3$ for the inclusion map. By definition of $P$, $i_P^\ast\omega^3=0$, and hence 
\[0=i_P^\ast(\dd\omega^3)=-i_P^\ast(\omega_1^3\wedge\omega^1+\omega_2^3\wedge\omega^2).\label{eq 2.3 Ivey}\]
By (\ref{eq 2.2 Ivey}), $i_P^\ast\omega^1$ and $i_P^\ast\omega^2$ are independent, and we can apply the Cartan Lemma~\ref{lem cartan} to the right hand side of (\ref{eq 2.3 Ivey}). We get 
\[i_P^\ast 
\begin{pmatrix}
    \omega_1^3 \\ \omega_2^3
\end{pmatrix}
...
\end{pmatrix},
\]
where $h_{ij}=h_{ji}$ are some functions defined on $P$ (they are related to the Hessian of the parametrizing immersion $\bm{x}:U\to \bbE^2$). Given an adapted (local) lift $\frake:\Sigma\to P$, we have 
\[\frake^\ast \begin{pmatrix}
    \omega_1^3 \\ \omega_2^3
\end{pmatrix}=
h_\frake \cdot \frake^\ast \begin{pmatrix}
    \omega^1 \\ \omega^2
\end{pmatrix},\quad h_\frake=\frake^\ast h.
\]
...
\[\wt\frake=\frake\cdot\begin{pmatrix}
    ...
    &1 &\\
    &&1
\end{pmatrix}\eqqcolon \frake\cdot R,
\]
where $R:\Sigma\to \SO_2$. We compare $\wt\frake^\ast \theta_{\SE_3}=\wt\frake^{-1}\dd \wt\frake$ with $\frake^\ast\theta_{\SE_3}=\frake^{-1}\dd\frake$. A direct substitution leads to 
...
Thus, the properties of $\frake$ that are invariant under conjugation by a rotation matrix are invariants of the parametrizing map $\bm{x}$. By the spectral theorem for symmetric matrices, the functions $H=\frac12 \tr(h_\frake)$ and $K=\det h_\frake$ generate the ideal of invariant functions of $h_\frake$. They are exactly the mean curvature and the Gauss curvature. We see immediately that any two congruent surfaces must have the same $K$ and $H$ at corresponding points.


The first and second fundamental forms can be canonically lifted to horizontal forms on $P$ (here the Kronecker tensor $\delta_{ij}$ is used in place of the standard metric on $\bbR^3$):
\begin{align}
    \wt{\rmI}&\coloneqq \delta_{ij} \omega^i\otimes\omega^j &\in\Gamma^\infty(\pi^\ast(\rmS^2 T^\ast \Sigma)),\\
    \wt{\rmII}&\coloneqq \omega^a_j\otimes\omega^j\otimes e_a&\in \Gamma^\infty(\pi^\ast(\rmS^2 T^\ast \Sigma\otimes N\Sigma)).
\end{align}

\begin{prop}
    $\wt{\rmII}$ descends to a well-defined tensor on $\Sigma$ with values in $NM$:
    \[\rmII(X,Y)=-\<\dd e_3(X),Y\>e_3\in\Gamma^\infty(\rmS^2T^\ast\Sigma\otimes N\Sigma).\]
\end{prop}

\begin{rem}
    Treat the sphere $\bbS^2$ as the homogeneous space $\SE_3\slash\SE_2$ via the projection $(\bm{x},e_1,e_2,e_3)\mapsto e_3$. As such, the form $\omega_1^3\wedge\omega_2^3$ is the pullback of the area form on $\bbS^2$ because $\dd e_3=-(\omega_1^3 e_1+\omega_2^3 e_2)$. Since $\omega_1^3\wedge\omega_2^3=K\omega^1\wedge\omega^2$, we may interpret $K$ as a measure of how much the area of $\Sigma$ is (infinitesimally) distorted under the Gauss map $e_3:\Sigma\to \bbS^2$.
\end{rem}
...
\[\dd\omega_1^2=-K\omega^1\wedge\omega^2.\label{eq 2.14 Ivey}\]
...
\begin{proof}[Proof of Theorema Egregium~\ref{thm egregium}]
    Let $\bm{x}:U\to \bbE^3$ be a local parametrization and let $\frake:U\to P$ be a first-order adapted lift. Let $\eta^j=\frake^\ast\omega^j$. Since $\eta^1\wedge\eta^2$ is nonvanishing, it spans $\bigwedge^2 T_p^\ast\Sigma$ at each $p\in\Sigma$. Thus, there exist functions $a,b$ such that 
    ...
    Now, if $\wt\frake$ is a different adapted lift with $\wt\eta^j=\wt\frake^\ast\omega^j$, then it is easy to show that $\dd\wt\alpha=\dd\alpha$. Thus the function $\kappa$ uniquely determined by $\dd\alpha=\kappa\eta^1\wedge\eta^2$ is also invariant under the change of the lift, and hence depends only on the Riemannian metric $(\eta^1)^2+(\eta^2)^2$.
...
\begin{align}
    \calR:\fX(M)\times\fX(M)\to \Gamma^\infty(V),\quad & \calR(X,Y)=-\calV[\calH X,\calH Y],\\
    \wb{\calR}:\fX(M)\times\fX(M)\to \Gamma^\infty(H),\quad & \wb{\calR}(X,Y)=-\calH[\calV X,\calV Y].
\end{align}
...
\[\iota_u^\vartheta\coloneqq \vartheta^{-1}\circ\iota_u:\bbR^n\to T_{\pi(u)}M,\quad \bm{x}\mapsto x^i u_i.\]
...
\begin{rem}\label{rem G-structure as soldering}
    Clearly, for a given $G$-structure $P$, we may pull back (or restrict) the soldering form $\theta$ of $\Fr M$ to $P$, and, thus, for any connection $\omega$ on $P$ we have a torsion $2$-form $\Theta$ on $P$ defined by (\ref{eq 2.1.10 RS2}). Denoting the reduction morphism by $i:P\to \Fr M$, the soldering form on $P$ is $i^\ast\theta$. For given $m\in M$ and $p\in P_m$, consider the map 
...
\begin{example}[Invariant connections on affine space]
...
If the Cartan geometry is not effective, then one cannot reconstruct the principal Cartan bundle from its associated bundles. However, every Cartan geometry has a family of bundles (and certain structures on them akin to soldering) naturally associated with it, which we now turn to. Recall our discussion of natural bundles following Definition~\ref{def natural bundle}. We showed that all natural bundles on the category of smooth manifolds are associated to a $k$-th order frame bundle. Now, in the context of the category $\Car_{(G,H)}$ of Cartan geometries, the most general definition of a natural bundle is a functor $\bm{F}:\Car_{(G,H)}\to \FB^\infty$, known as a \emph{gauge natural bundle}. 

This definition can be made much more concrete. Given a gauge natural bundle $\bm{F}$ on $\Car_{(G,H)}$, we can apply it to the homogeneous model $(G\overset{\pi}{\to} G\slash H,\theta_G)$. Since the left translations $L_g,g\in G$, are exactly the automorphisms of this Cartan geometry, $\bm{F}$ turns them into automorphisms of $\bm{F}(G)$, turning it into a \emph{homogeneous bundle} (as discussed in Section~\ref{sec: homogeneous bundles}). By Proposition~\ref{prop 1.4.3 Cap}, we get an $H$-action $\sigma$ on the fiber $F\coloneqq \bm{F}(G)_{[e]}$, which determines the homogeneous bundle $\bm{F}(G)$ up to isomorphism. This allows us to reconstruct the entire functor $\bm{F}$ as the association functor for the action $\sigma$, which we denote as usual by $[\sigma]$. In other words, there is an equivalence between gauge natural bundles and functors of the form $[\sigma]:(P\to M,\eta)\mapsto P^{[\sigma]}=P\times_H F$. This leads to the following definition.
