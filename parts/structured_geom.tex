...
\begin{xca}
    ...
    \[\kappa=\left|\<\rmII(\dot\gamma,\dot\gamma),e_3\>\right|=|Q(\dot\gamma,\dot\gamma|.\]
\end{xca}
...
\begin{example}[Lagrangian submanifolds]
...
\begin{cor}[Existence of isothermal coordinates]\label{cor isothermal coords}
...
In this section we apply the methods of hyperbolic \gls{eds} to study a special class of surfaces related to the sine-Gordon equation. But first we need to briefly discuss curves on surfaces.
...
Substituting $\dd u=u_x\dd x+u_y\dd y$ into the $2$-forms of the \gls{eds} shows that $\omega_2^1=u_x\dd x-u_y\dd y$ along any integral surface. Then it is easy to see that the structure equation $\dd\omega^1_2=\omega^3_1\wedge\omega^3_2$ implies that $u$ (or rather $2u$) satisfies the sine-Gordon equation:
\[u_{xy}=\sin u\cos u.\]

Conversely, we may start with a solution of the sine-Gordon equation and produce a pseudospherical surface by integration. On a fundamental level, note that the First and Second Fundamental Forms of a pseudospherical surface in the asymptotic coordinates $(x,y)$ read 
\[
    \rmI=\begin{pmatrix}
        1 &\cos 2u\\
        \cos 2u & 1
    ...
    \end{pmatrix},
\]
and $2u$ is the angle between the $x$ and $y$-curves. From here it is easy to see that $H=-2\cot(2u)$. The sine-Gordon equation is equivalent to the Gauss-Kodazzi equations on $H$ with $K=-1$, so by the Fundamental Theorem of Surfaces, each solution of the sine-Gordon equation determines a unique immersed surface up to a rigid motion. 

\begin{figure}[tp]
    \centering
    \includegraphics[scale=0.5]{figures/SG1soliton.png}
    \caption{The $1$-soliton solution (\ref{eq SGE 1-soliton}) (with lines of constant $y$) of the sine-Gordon equation and the corresponding pseudospherical surfaces for different values of $\lambda^2$. The first three surfaces have the lines of curvature marked on them. The rightmost is the pseudosphere $\lambda=1$ with the asymptotic lines marked on it. Note the asymptotic lines becoming tangent at the edge of the surface.}
...
\]
This is an overdetermined system for the matrix $\frake(x,y)$, and its Frobenius integrability condition is the sine-Gordon equation for $u$. In fact, the above pair of equations is a $UV$-pair for the sine-Gordon equation (see Example~\ref{ex UV pairs}). So, given a solution to sine-Gordon, we may obtain the framing $\frake$ by solving linear systems of ODE's, and then solve for the surface $\bf{x}(x,y)\in\bbR^3$ by integrating 
...
\begin{example}[Hyperbolic plane and disk]
    ...
    \[\sfg=\frac{|\dd z|^2}{(1-|z|^2)^2}=\frac{\dd x^{\otimes 2}+\dd y^{\otimes 2}}{(1-x^2-y^2)^2}.\]
    ...
    Since this is a M\"obius transformation, it takes circles and lines to circles and lines. As a consequence, the geodesics of the disk model are diametric lines and arcs of circles perpendicular to the bounding circle, see Figure~\ref{fig:hyperbolic plane}.
\end{example}
...
\begin{proof}
    ...
    The Uniformization Theorem (in one of its formulations) states that the universal covering of any complete surface $\Sigma$ of constant negative curvature, say $K\equiv -1$, together with the pullback of the metric from $\Sigma$, is isometric to the hyperbolic plane $\bbC^+$. Now, if we prove that $\bbC^+$ cannot be isometrically immersed into $\bbE^3$, Hilbert's theorem will follow because the universal covering map $\pi:\bbC^+\to \Sigma$ is a local isometry, and so any isometric immersion  $\bf{x}:\Sigma\to \bbE^3$ would allow us to define an isometric immersion $\bf{x}\circ \pi:\bbC^+\to \bbE^3$ (note that we never required the immersions to be injective), leading to a contradiction.

    Now let $2u$ be the solution of the sine-Gordon equation $(2u)_{xy}=\sin 2u$ correspoding to $\bf{x}(\bbC^+)$. Here $(x,y)$ are the asymptotic coordinates on $\bf{x}(\bbC^+)$, not to be confused with the Cartesian coordinates on $\bbC^+$. Since $\bf{x}$ is an immersion, this surface must have linearly independent asymptotic directions at each point, and hence 
    ...
    \end{multline}
    Since $\bbC^+$ is complete and simply connected, its asymptotic coordinates are globally defined, so the rectangle $R$ can be made arbitrarily large. Since the area of $\bbC^+$ is infinite, the area of $R$ can become arbitrarily large. This contradicts the above result showing that $\bf{x}(R)$ has bounded area, since $\bf{x}$ is supposed to be an isometry.
\end{proof}
...
\begin{cor}[Weiestrass-Enneper representation]\index{Weierstrass-Enneper representation}
    Each nonplanar minimal surface $\Sigma<\bbE^3$ is determined, up to rigid motions, by its Weierstrass data consisting of a meromorphic function $g:\Sigma\to \CP^1$ and a $1$-form $\theta\in\Omega^1(\Sigma;\bbC)$ that is holomorphic with respect to the complex structure on $\Sigma$ induced by the metric and orientation. Conversely, every such pair $(g,\theta)$ determines a minimal surface as long as the poles of $g$ overlap with zeros of $\theta$ so that $g^2\theta$ is holomorphic.
\end{cor}

\begin{defn}[Conjugate surface]
    Given a minimal surface $\bf{x}:\Sigma\to \bbE^3$ with Weierstrass-Enneper representation $(g,\theta)$, its conjugate surface is the surface obtained by multiplying $\theta$ by $\rmi$, or equivalently, by replacing $\Re$ with $\Im$ in (\ref{eq Weiestrass-Enneper}).
\end{defn}
...
\begin{example}[Catenoid and helicoid]
    ...
    \[f(z)=-\i a\rme^{-\i z},\quad g(z)=a\rme^{\i z},\quad a>0,\]
    and its conjugate surface is the helicoid (although a slight reparametrization will be necessary to bring to the same form as in Example~\ref{ex helicoid}). The rest of this $1$-parameter family of surfaces is shown in Figure~\ref{fig:helicoid}.
\end{example}

We also can observe that multiplying $f$ by a unit complex number $\rme^{\i t}$, $t\in (0,2\pi)$, will produce a one-parameter family of isometric minimal surfaces. The catenoid and helicoid are members of one such family.

\begin{xca}
    \begin{enumerate}
        \item Identify the surfaces corresponding to $f=1$ and $g$ being one of $0,z,z^{-1},\i z^{-1}$. 
...
\begin{prop}[{{\cite[Prop.~7.4.25]{Ivey}}}]\label{prop 7.4.25 Ivey}
    ...
    Then the \emph{Hopf differential}
    \[Q\coloneqq q(\eta^1+\i \eta^2)^{\otimes 2}\]
    is a well-defined holomorphic quadratic differential on $\Sigma$ and it coincides with the traceless part of the $Q$ defined in \Subsect~\ref{sec: theory of surfaces}. Moreover, for any constant $H$, $q$ determines a unique local isometric embedding of $\Sigma$ as a \gls{cmc} surface of mean curvature $H$ into $\bbE^3$ up to rigid motions.
\end{prop}
\begin{proof}
...
\begin{rem}
    On minimal surfaces, the Second Fundamental Form is traceless, so $Q$ coincides with the full $Q$ as defined in \Subsect~\ref{sec: theory of surfaces}. Thus, minimal isometric immersions are completely parametrized by a holomorphic quadratic differential.
\end{rem}
...
Equations which are integrable by the method of Darboux give one example where we can construct integral manifolds of an \gls{eds} by restricting to a submanifold where the system becomes Frobenius. Another example arises when the integrability condition for an overdetermined system of PDE can be reduced to finding the solution of another PDE. Then any solution of that PDE enables us to construct (using ODE techniques) a solution of the system.

\begin{example}\label{ex 7.5.1 Ivey}
    For example, the $UV$-pair representation (\ref{eq 7.21 Ivey}) of the sine-Gordon equation enables us to construct a solution to the \gls{ma} system for pseudospherical surfaces by integrating a Frobenius system.
\end{example}

When this transformation works in both directions, the two PDE's are said to be related by a B\"acklund transformation, which we will define later in the section. First, we formalize the relationship between the systems seen in the above examples.
...
\begin{rem}\label{rem 7.5.4 Ivey}
    Suppose $\calI$ is a \gls{eds} on a manifold $M$ with a $2$-form $\Phi\in\calI^2$ closed. Then, by the Poincar\'e Lemma, in some contractible neighborhood of any given point in $M$ there exists a differential form $\phi$ such that $\dd\phi=\Phi$. We may define a rank-$1$ integrable extension of $\calI$ by introducing a new coordinate $y$ and letting the Pfaffian system $J$ on $U\times\bbR$ be spanned by $\dd y-\phi$. Because such forms $\Phi$ are examples of conservations laws for an \gls{eds}, we call this construction an \emph{extension via conservation law}.
\end{rem}
...
\begin{example}\label{ex 7.5.6 Ivey}
    Parametric families of integrable extensions are frequently encountered in the study of completely integrable PDE's, i.e.\ \emph{solitonic equations}. We now describe the integrable extensions of the \gls{kdv} equation 
...
B\"acklund transformations are a particular form of integrable extension which allows a two-way transformation of solutions of a PDE to solutions of another (or the same) PDE. One of the most well-known is the following transformation, which can produce new solutions of the sine-Gordon equation.
...
\begin{example}\label{ex 7.5.9 Ivey}
    ...
    \[u=2\arctan \frac{\lambda_1+\lambda_2}{\lambda_1-\lambda_2}\frac{\rme^{\lambda_1x+\lambda_1^{-1}y}-\rme^{\lambda_2 x+\lambda_2^{-1}y}}{1+\rme^{(\lambda_1+\lambda_2)x+(\lambda_1^{-1}+\lambda_2^{-1})y}}.\]
...
\begin{figure}[tp]
    \centering
    \includegraphics[scale=0.5]{figures/SG2soliton.pdf}
    \caption{$2$-soliton solution of the sine-Gordon equation (height profiles at constant $y$) and the corresponding pseudospherical surface.}
    \label{fig:2-soliton}
\end{figure}
...
\begin{example}[\ref{ex 7.5.9 Ivey} continued]
    Let $M_1$ and $M_2$ be two copies of $J^1(\bbR^2,\bbR)$ where we denote the forms and coordinates on $M_2$ with bars. For $i=1,2$ let the \gls{eds} $\calI_i$ on $M_i$ be a copu of the \gls{ma} system for the sine-Gordon equation from Example~\ref{ex 7.4.6 Ivey}. For any $\lambda\neq 0$, let $B_\lambda<M_1\times M_2$ be defined by $\wb{x}=x$, $\wb{y}=y$ and 
    \begin{align}
...
\begin{figure}[tp]
    \centering
    \includegraphics[scale=0.5]{figures/SGbreather.pdf}
    \caption{A breather wave solution of the sine-Gordon equation and the corresponding pseudospherical surface.}
    \label{fig:breather}
\end{figure}
