...
\begin{figure}[tp]
    \centering
    \includegraphics[scale=0.5]{figures/helicoid.jpeg}
    \caption{A $1$-parameter family of minimal ($H=0$) isometric immersions of a surface connecting the helicoid (top left) to the catenoid (bottom right).}
    \label{fig:helicoid}
\end{figure}
...
A theorem of Hilbert states that pseudospherical surfaces in Euclidean space cannot be geodesically complete (i.e.\ geodesics terminate in finite time). The standard proof proceeds by showing that the area of \emph{any} net parallelogram (of any size) has area bounded by a constant. However, a simply connected complete surface of constant negative curvature has necessarily infinite area, which leads to a contradiction. This was one of the earliest global results in differential geometry.
