...
\begin{defn}[$n$-compressible pair]\index{$n$-compressible space}
    ...
    \begin{tikzcd}[every matrix/.append style={name=m}, row sep=large, column sep=large]
       ...
       \bbD^n \arrow[r,"\varPhi", swap]\arrow[ur,"\Psi",dashed,swap] & Y
    \end{tikzcd}
    \]
    there exists $\Psi:\bbD^n\to X$ such that $\restr{\Psi}{\partial \bbD^n}=\varphi$ and $f\circ \Psi\simeq \varPhi$ $\rel \partial \bbD^n$ (this amounts to (3) of Proposition~\ref{prop 6.7.3 tomDieck}). This notion is invariant under compositions with homotopy equivalences of the target space $Y$.
\end{defn}
