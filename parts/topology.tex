...
\begin{defn}[Separation Axioms]\index{Separation axioms}
    ...
    \begin{enumerate}
        \item $T_1$: There exists an open neighborhood $U$ of $x$ that doesn't include $y$, and an open neighborhood $V$ of $y$ that doesn't include $x$, i.e., $x$ and $y$ are \emph{separated}.
...
\begin{cor}[Fundamental group of a wedge sum {{\cite[Theorem 10.7]{LeeTop}}}]
...
\begin{enumerate}
    ...
    \item All Lie groups (defined as smooth manifolds with a group structure in which multiplication and inversion are smooth maps) in fact support a unique (up to diffeomorphism) $C^\omega$ structure. This is an immediate consequence of the convergence of the Baker-Campbell-Hausdorff series.
\end{enumerate}
\end{example}
...
In this \subsect\ we just recall the statements of two basic theorems of multivariate calculus.
...
\begin{proof}
    ...
    7) Consider the inclusion map $i:\bbR^n_+\hookrightarrow \bbR^n$. Clearly $i_{\ast 0}=\id$ is invertible, but $i$ itself is not a local diffeomorphism because $i(B^n\cap \bbR^n_+)$ is the image of an open set that is not open in $\bbR^n$.
\end{proof}
