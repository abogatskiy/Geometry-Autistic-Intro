...
\begin{example}
    ...
    $n$ being the size of the matrix. $\Mat_n(n,R)$ forms an associative
...
\end{xca}
\begin{example}
    If we restrict to invertible matrices the associative ring $\Mat_n(n,R)$
...
\begin{thm}[{{\cite[Thm.~5.31]{Lee}}}]\label{thm 5.31 Lee}
    If $S\sub M$ is embedded, then the subspace topology on $S$ and the smooth structure defined by slice charts are the only topology and smooth structure w.r.t.\ which $S$ is an embedded or even immersed manifold.
\end{thm}
